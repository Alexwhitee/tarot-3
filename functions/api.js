...
- **story_hint（情境脉络）**：权重40%，核心情境线索
- **actions（动作）**：权重30%，具体动作或事件发生方式
...
**核心象征选择**：
- 选择：[象征A] → [现实对应A]
- 理由：[分析此象征与占卜问题的核心关联度，说明它为何比其他候选象征更能主导事件走向，提供牌面证据]
- 排除：[象征B]（原因：[说明此象征与问题关联度较低，或在当前情境下属于次要影响/背景因素]）
...
\`\`\`
**核心象征选择**：
- 选择：宝剑骑士的冲锋姿态 → 求职者的积极行动
- 理由：牌面显示骑士策马疾驰，与问题中"是否能找到工作"的主动求职行为高度匹配，是推动事件发展的主要动力，远强于其他象征
- 排除：背景的乌云（原因：属于环境阻力，非求职者本身状态，且在画面中占比较小，属次要影响因素）

**条件分支分析**：
| 情况 | 倾向强度 | 描述 | 触发/风险因素 |
|------|---------|------|--------------|
| 情况1 | 高可能性(70%) | 快速获得面试机会 | 积极性导致，但准备不足可能失败 |
| 情况2 | 低可能性(30%) | 错过合适机会 | 行动过于急躁，缺乏策略性 |
\`\`\`

**操作要点**：

- 从候选象中选出最符合现实的核心象征
- 分析主导性：说明所选象征为何比其他候选更能决定事件走向
- 优先考虑现实匹配度而非正负面倾向
- 理由必须基于牌面证据，形成A→B→C的因果链；避免循环（如"因为积极所以成功"）
- 多重可能性时使用条件分支分析
- 说明未选取象征的对应偏差
...
\`\`\`
**发展脉络**：
现状位的宝剑骑士显示求职者正处于积极行动期，投递简历频繁但缺乏策略。这导致
阻碍位的五角星四逆位表明过度关注薪资待遇，可能错过发展机会。若不调整心态，将引发
结果位的圣杯二显示最终会获得一个需要合作的职位，但薪资可能低于预期。

**关键节点**：
- 触发条件：调整薪资期望，重视团队合作能力
- 潜在阻力：对待遇的固执态度可能导致谈判破裂
- 不可控因素：市场竞争激烈，优质岗位稀缺
\`\`\`
...
\`\`\`
**问题答案**：能够找到工作，但过程会有波折
**倾向结果**：高可能性（约70%）在2个月内获得职位，但薪资可能比预期低20-30%
**触发条件**：降低薪资期望，重视团队协作机会
**风险规避**：
- 主要风险：因薪资谈判过于强硬导致机会流失
- 预防措施：提前调研市场薪资水平，准备薪资谈判的底线和弹性空间
**时间预估**：1-2个月内（宝剑牌显示快速进展）会有明确结果
...
\`\`\`
基于核心象征创作4句诗，保持意象具体性的同时明确指向结果倾向。
\`\`\`
